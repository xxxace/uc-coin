...
export interface UpdatePledgeCfgProps {
...
export function updatePledgeCfg({ pledgeToken, payToken, profitToken, pledgeReturn, pledgeDays }: UpdatePledgeCfgProps) {
    const { config } = usePrepareContractWrite({
        address: contract_address,
        abi: wagmigotchiABI,
        functionName: 'updatePledgeCfg',
        args: [
...
export default function UpdatePledgeCfg() {
    ...
    const { data, isLoading, isSuccess, isError, write, error } = updatePledgeCfg({ pledgeToken, payToken, profitToken, pledgeReturn, pledgeDays })
    ...
    return (
        <div className={styles.borderedDiv} >
            <p>
                <span className={styles.boldText}>UpdatePledgeCfg 更新配置质押Token(721), 支付token，收益token，收益类型，质押天数</span>。
            </p>
            <div>
            ...
            </div>
            {
                <>
                    ...
                    <button className="disabled:opacity-75 disabled:cursor-not-allowed" disabled={!write || isLoading} onClick={() => write && write()}>
                        {isLoading ? 'Writing...' : 'UpdatePledgeCfg'}
                    </button>
                    {isSuccess && (
                        <div>
                            Successfully UpdatePledgeCfg!
                            <div>
