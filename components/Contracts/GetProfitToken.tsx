...
export function getProfitToken(tokenAddr: `0x${string}`, pledgeDays: number) {
    return useContractRead({
        address: contract_address,
        abi: wagmigotchiABI,
        functionName: 'getProfitToken',
        args: [tokenAddr, pledgeDays]
    })
}

export default function GetProfitToken() {
    const [tokenAddr, setTokenAddr] = useState('')
    const [pledgeDays, setPledgeDays] = useState(0)

    const { data, isLoading, isError } = getProfitToken(tokenAddr as `0x${string}`, pledgeDays)

    return (
        <div className={styles.borderedDiv} >
            <p>
                <span className={styles.boldText}>返回收益token信息</span>。
            </p>
            <label>
                Token Address:
